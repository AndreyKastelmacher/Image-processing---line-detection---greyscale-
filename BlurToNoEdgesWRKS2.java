...
import java.util.Arrays;
    ...
    public class BlurToNoEdgesWRKS2 {
	    ...
	    */  
	    ...
	    public static void normalize(int[][] source ){
	    	int[][] outArray = new int[source.length][source[0].length];
	    ...
	    public static int getNorm(int pixel, double min , double max ){
	    	...
	    	double out = (pixel-min)*(((255))/max-min);
	    	int out1 = (int)Math.round(out);
	    	//out1 = 254 - out1;
	    ...
	    public static int findMin(int[][]source){
	    	int min = source[0][0];
	    	for (int i1 = 1; i1 < source.length-1; i1++ ){
	        	for (int j1 =1; j1 < source[0].length-1; j1++ ){
	        	    if( min > source[i1][j1] ){
	        	    	min = source[i1][j1]; 
	        	    }
	        	}    
	    ...
	    public static int[][][] matrixCompiler(int[][] gradient , int[][][] target){
	        for (int i = target.length-2; i > 0 ; i-- ){
	            for (int j = target[0].length-2; j > 0 ; j-- ){
